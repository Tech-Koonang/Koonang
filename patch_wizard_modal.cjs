...
const data = `import { AnimatePresence, motion } from "framer-motion";
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {options.purposes.map((p: any) => (
              <label key={p.value} className={\`flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-all \${formData.purposes.includes(p.value) ? 'border-tosca bg-tosca/5 dark:bg-tosca/10' : 'border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20'}\`}>
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {options.integrations.map((i: any) => (
              <label key={i.value} className={\`flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-all \${formData.integrations.includes(i.value) ? 'border-firefly bg-firefly/5 dark:bg-firefly/10' : 'border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20'}\`}>
...
`;
fs.writeFileSync('src/components/wizard/wizard-modal.tsx', data, 'utf8');
