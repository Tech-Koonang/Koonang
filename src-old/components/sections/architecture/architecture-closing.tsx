...
export function ArchitectureClosing({ onOpenWizard }: { onOpenWizard: () => void }) {
  const { t } = useLanguage();

  return (
    <section className="py-32 bg-white dark:bg-navy-900 transition-colors duration-500 relative overflow-hidden">
      {/* Background Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-firefly/10 blur-[120px] rounded-full" />
      ...
      <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
            ...
            >
              {t.architecturePage.closing.ctaPrimary}
            </motion.button>
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            ...
            >
              {t.architecturePage.closing.ctaSecondary}
            </motion.button>
          </div>
        </motion.div>
      </div>
