    ...
    <main className="min-h-screen bg-slate-50 dark:bg-navy-900 transition-colors duration-500 overflow-hidden relative pb-32">
      ...
      <div className="pt-32 max-w-4xl mx-auto px-6 relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
          ...
          >
            <Download size={18} />
            {isGeneratingPDF ? 'Generating PDF...' : safeT.resultPage.booking.downloadPDF}
          </button>
        </motion.div>
        ...
        <div id="pdf-content" className="space-y-8 mb-16">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="bg-white dark:bg-navy-800 border border-slate-200 dark:border-white/10 rounded-2xl p-6 md:p-8">
            <h2 className="text-xl font-bold text-navy-900 dark:text-white mb-6">{safeT.resultPage.inquiryInfo?.title || 'Inquiry Information'}</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <tbody>
                <tr className="border-b border-slate-200 dark:border-white/10">
                  <td className="py-3 pr-4 font-medium text-slate-700 dark:text-slate-300 w-1/3">{safeT.resultPage.inquiryInfo?.fullName || 'Full Name'}</td>
                  <td className="py-3 text-slate-600 dark:text-slate-400">{inquiry.full_name}</td>
                </tr>
                <tr className="border-b border-slate-200 dark:border-white/10">
                  <td className="py-3 pr-4 font-medium text-slate-700 dark:text-slate-300">{safeT.resultPage.inquiryInfo?.email || 'Email'}</td>
                  <td className="py-3 text-slate-600 dark:text-slate-400">{inquiry.email}</td>
                </tr>
                <tr className="border-b border-slate-200 dark:border-white/10">
                  <td className="py-3 pr-4 font-medium text-slate-700 dark:text-slate-300">{safeT.resultPage.inquiryInfo?.phone || 'Phone'}</td>
                  <td className="py-3 text-slate-600 dark:text-slate-400">{inquiry.phone}</td>
                </tr>
                {inquiry.business_name && (
                  <tr className="border-b border-slate-200 dark:border-white/10">
                    <td className="py-3 pr-4 font-medium text-slate-700 dark:text-slate-300">{safeT.resultPage.inquiryInfo?.businessName || 'Business Name'}</td>
                    <td className="py-3 text-slate-600 dark:text-slate-400">{inquiry.business_name}</td>
                  </tr>
                )}
                {inquiry.industry && (
                  <tr className="border-b border-slate-200 dark:border-white/10">
                    <td className="py-3 pr-4 font-medium text-slate-700 dark:text-slate-300">{safeT.resultPage.inquiryInfo?.industry || 'Industry'}</td>
                    <td className="py-3 text-slate-600 dark:text-slate-400">{inquiry.industry}</td>
                  </tr>
                )}
                {inquiry.website && (
                  <tr className="border-b border-slate-200 dark:border-white/10">
                    <td className="py-3 pr-4 font-medium text-slate-700 dark:text-slate-300">{safeT.resultPage.inquiryInfo?.website || 'Website'}</td>
                ...
                {inquiry.purposes && inquiry.purposes.length > 0 && (
                  <tr className="border-b border-slate-200 dark:border-white/10">
                    <td className="py-3 pr-4 font-medium text-slate-700 dark:text-slate-300">{safeT.resultPage.inquiryInfo?.purposes || 'Purposes'}</td>
                    <td className="py-3 text-slate-600 dark:text-slate-400">{inquiry.purposes.join(', ')}</td>
                  </tr>
                )}
                {inquiry.challenges && (
                  <tr className="border-b border-slate-200 dark:border-white/10">
                    <td className="py-3 pr-4 font-medium text-slate-700 dark:text-slate-300 align-top">{safeT.resultPage.inquiryInfo?.challenges || 'Challenges'}</td>
                    <td className="py-3 text-slate-600 dark:text-slate-400 whitespace-pre-wrap">{inquiry.challenges}</td>
                  </tr>
                )}
                {inquiry.growth_target && (
                  <tr className="border-b border-slate-200 dark:border-white/10">
                    <td className="py-3 pr-4 font-medium text-slate-700 dark:text-slate-300 align-top">{safeT.resultPage.inquiryInfo?.growthTarget || 'Growth Target'}</td>
                ...
                {inquiry.integrations && inquiry.integrations.length > 0 && (
                  <tr>
                    <td className="py-3 pr-4 font-medium text-slate-700 dark:text-slate-300">{safeT.resultPage.inquiryInfo?.integrations || 'Integrations'}</td>
