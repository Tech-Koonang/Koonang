...
import { AlertCircle, Layers, Network, Database } from "lucide-react";

const icons = [AlertCircle, Layers, Network, Database];
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <h2 className="text-3xl md:text-5xl font-bold text-navy-900 dark:text-white mb-6">
            {t.architecturePage.problems.headline}
          </h2>
        </motion.div>

        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
          {t.architecturePage.problems.cards.map((card, i) => {
            const Icon = icons[i];
            return (
              <motion.div
                key={i}
        ...
        >
          <p className="text-2xl md:text-3xl font-bold text-navy-900 dark:text-white italic">
            {t.architecturePage.problems.closing}
          </p>
        </motion.div>
      </div>
    </section>
